...
const getButtonName = path => path === "/" ? "New Election" : path === "/polls" ? "New Poll" : path === "/survey" ? "New Survey" : "New Quiz"


const Navbar = () => {
    const { pathname } = useLocation()
    const location = getButtonName(pathname)
        ...
        <div>
            ...
            --> */}
                            ...
                            <div class="hidden md:ml-6 md:flex md:items-center">
                                <Link to="/" className={`${pathname === "/" ? "bg-gray-900" : ""} ml-4 px-3 py-2 rounded-md text-sm font-medium leading-5 text-gray-300 hover:text-white hover:bg-gray-700 focus:outline-none focus:text-white focus:bg-gray-700 transition duration-150 ease-in-out`}>Election</Link>
                                <Link to="/polls" className={` ${pathname === "/polls" ? "bg-gray-900" : ""} ml-4 px-3 py-2 rounded-md text-sm font-medium leading-5 text-gray-300 hover:text-white hover:bg-gray-700 focus:outline-none focus:text-white focus:bg-gray-700 transition duration-150 ease-in-out`}>Polls</Link>
                                <Link to="/survey" className={`${pathname === "/survey" ? "bg-gray-900" : ""} ml-4 px-3 py-2 rounded-md text-sm font-medium leading-5 text-gray-300 hover:text-white hover:bg-gray-700 focus:outline-none focus:text-white focus:bg-gray-700 transition duration-150 ease-in-out`}>Surveys</Link>
                                <Link to="/quiz" className={`${pathname === "/quiz" ? "bg-gray-900" : ""} ml-4 px-3 py-2 rounded-md text-sm font-medium leading-5 text-gray-300 hover:text-white hover:bg-gray-700 focus:outline-none focus:text-white focus:bg-gray-700 transition duration-150 ease-in-out`}>Quizes</Link>
                            </div>
                        </div>
                        <div class="flex items-center">
                            <div class="flex-shrink-0">
                                <span class="rounded-md shadow-sm">
                                    <button onClick={handleViewDrawer} type="button" class="relative inline-flex items-center px-4 py-2 border border-transparent text-sm leading-5 font-medium rounded-md text-white bg-indigo-500 hover:bg-indigo-400 focus:outline-none focus:shadow-outline-indigo focus:border-indigo-600 active:bg-indigo-600 transition duration-150 ease-in-out">
                                        {/* <!-- Heroicon name: plus --> */}
                            ...
                            <div class="hidden md:ml-4 md:flex-shrink-0 md:flex md:items-center">
                                <button class="p-1 border-2 border-transparent text-gray-400 rounded-full hover:text-gray-300 focus:outline-none focus:text-gray-500 focus:bg-gray-100 transition duration-150 ease-in-out" aria-label="Notifications">
                                    {/* <!-- Heroicon name: bell --> */}
  ...
  --> */}
            ...
            <Drawer visible={createElection} width="40%" onClose={handleViewDrawer} destroyOnClose>
                <CreateElection setCreateElection={setCreateElection} />
            </Drawer>

            <Drawer visible={createPoll} width="40%" onClose={handleViewDrawer}>
                <CreatePoll setCreatePoll={setCreatePoll} />
            </Drawer>

            <Drawer visible={createSurvey} onClose={handleViewDrawer}>

            </Drawer>

            <Drawer visible={createQuiz} onClose={handleViewDrawer}>

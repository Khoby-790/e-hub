...
import { UPDATE_ELECTION } from '../../redux/actions/types'
import { BsX } from 'react-icons/bs'
import { message } from 'antd'


const UpdateElection = ({ setCreateElection, election, id }) => {
    ...
    useEffect(() => {
        if (election) {
            ...
            setPositions(election.positions.length)
    ...
    const handleSubmitElection = () => {
        ...
        _dispatch({ type: UPDATE_ELECTION, payload: election })
        setCreateElection(prev => !prev);
        message.success('Election slot updated Successfully');
    ...
    return (
        <div className="mt-10">
            <div className="mb-3">
                <h1 className="font-bold text-lg">Update Election</h1>
            </div>
            <div className="mb-2">
                <label htmlFor="email" class="">Name</label>
                <div class="relative rounded-md shadow-sm">
                    <input id="email" value={name} onChange={e => setName(e.target.value)} class="form-input block w-full sm:text-sm sm:leading-5" placeholder="you@example.com" />
                </div>
            </div>
            <div className="mb-2">
                <label htmlFor="email" class="">Date</label>
                <div class="relative rounded-md shadow-sm">
                    <input type="date" value={date} onChange={e => setDate(e.target.value)} id="email" class="form-input block w-full sm:text-sm sm:leading-5" placeholder="you@example.com" />
                </div>
            </div>
